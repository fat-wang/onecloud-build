...
clang_file="clang+llvm-14.0.5-armv7a-linux-gnueabihf.tar.xz"
#
# Set font color
...
compile_kernel() {
    ...
    MAKE_SET_STRING=" ARCH=${ARCH} CC=${CC} LD=${LD} V=1 LLVM=1 LLVM_IAS=1 LOCALVERSION=${LOCALVERSION} -mfpu=neon "
...
packit_kernel() {
    ...
    # cp -f ${kernel_path}/${local_kernel_path}/arch/arm/boot/dts/allwinner/*.dtb . && chmod +x * && sync
    ...
    # cp -f ${kernel_path}/${local_kernel_path}/arch/arm/boot/dts/amlogic/*.dtb . && chmod +x * && sync
    ...
    # cp -f ${kernel_path}/${local_kernel_path}/arch/arm/boot/dts/rockchip/*.dtb . && chmod +x * && sync
    # tar -czf dtb-rockchip-${kernel_outname}.tar.gz * && sync
    # mv -f *.tar.gz ${out_kernel}/${kernel_version} && sync
    # echo -e "${SUCCESS} The [ dtb-rockchip-${kernel_outname}.tar.gz ] file is packaged."
    
    cd ${out_kernel}/dtb/amlogic
    cp -f ${kernel_path}/${local_kernel_path}/arch/arm/boot/dts/meson8b*.dtb . && chmod +x * && sync
    tar -czf dtb-${kernel_outname}.tar.gz * && sync
    mv -f *.tar.gz ${out_kernel}/${kernel_version} && sync
    echo -e "${SUCCESS} The [ dtb-${kernel_outname}.tar.gz ] file is packaged."
...
[[ "${arch_info}" == "armv7" ]] || error_msg "The script only supports running under Armbian system."
